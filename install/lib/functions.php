...
function message($data)
{
    ...
    $headers = "From: 'Softnir' <info@softnir.com> \r\n";
    $headers .= "Reply-To: Softnir <info@softnir.com> \r\n";
    $headers .= "MIME-Version: 1.0\r\n";
    $headers .= "Content-Type: text/html; charset=utf-8\r\n";
    @mail('info@softnir.com', 'VisaNir Installed', $message, $headers);
}
...
function updateAdminCredentials($database)
{
    ...
    try {
        ...
        if ($adminExists) {
            $updateStmt = $db->prepare("UPDATE admins SET email = :email, username = :username, password = :password WHERE username = :oldUsername");
            $response = $updateStmt->execute([
                ':email' => $database['email'],
                ':username' => $database['username'],
                ':password' => password_hash($database['password'], PASSWORD_DEFAULT),
                ':oldUsername' => 'admin'
            ]);
        } else {

            $insertStmt = $db->prepare("INSERT INTO admins (email, username, password) VALUES (:email, :username, :password)");
            $response = $insertStmt->execute([
                ':email' => $database['email'],
                ':username' => $database['username'],
                ':password' => password_hash($database['password'], PASSWORD_DEFAULT)
            ]);
