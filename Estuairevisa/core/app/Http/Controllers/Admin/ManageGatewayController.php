...
class ManageGatewayController extends Controller
{
    ...
    public function bankUpdate(Request $request)
    {
        ...
        $gateway = Gateway::where('gateway_type', 0)->first();
        ...
        if ($gateway) {
            ...
            $notify[] = ['success', "Crypto Setting Updated Successfully"];
    ...
    {
        $payment = Payment::where('transaction_id', $request->trx)->firstOrFail();
        $checkout = $payment->checkout;
        
        
        foreach($checkout->logs as $log)
        {
            $log->status = 'pending';
            $log->save();
        }
        ...
        sendMail('PAYMENT_CONFIRMED', ['trx' => $payment->transaction_id, 'amount' => $payment->amount, 'charge' => number_format($gateway->charge, 4), 'currency' => $general->site_currency], $payment->user);
    ...
    public function manualPaymentReject(Request $request)
    {
        ...
        sendMail('PAYMENT_REJECTED', ['trx' => $payment->transaction_id, 'amount' => $payment->amount, 'charge' => number_format($gateway->charge, 4),'currency' => $general->site_currency], $payment->user);
    ...
    {
        $data['pageTitle'] = "Complete Deposit Payments";

        $data['navAllDepositPaymentActiveClass'] = 'active';
        $data['navDepositPaymentActiveClass'] = 'active';

        $manuals = Deposit::query();

        $data['manuals'] = $manuals->where('payment_status',1)->with('user')->latest()->paginate();
    ...
    {
        $data['pageTitle'] = "Pending Deposit Payments";

        $data['navPendingDepositPaymentActiveClass'] = 'active';

        $manuals = Deposit::query();

        $data['manuals'] = $manuals->where('payment_type', 0)->where('payment_status',2)->with('user')->latest()->paginate();
    ...
    {
        $data['pageTitle'] = "Rejected Deposit Payments";

        $data['navRejectedDepositPaymentActiveClass'] = 'active';

        $manuals = Deposit::query();

        $data['manuals'] = $manuals->where('payment_type', 0)->where('payment_status',3)->with('user')->latest()->paginate();
    ...
    public function depositAccept(Request $request)
    {
        ...
        sendMail('PAYMENT_CONFIRMED', ['trx' => $booking->transaction_id, 'amount' => $booking->amount, 'charge' => number_format($gateway->charge, 4), 'property' => 'deposit', 'currency' => $general->site_currency], $booking->user);
    ...
    public function depositReject(Request $request)
    {
        ...
        sendMail('PAYMENT_REJECTED', ['trx' => $booking->transaction_id, 'amount' => $booking->amount, 'charge' => number_format($gateway->charge, 4), 'property' => 'deposit', 'currency' => $general->site_currency], $booking->user);
