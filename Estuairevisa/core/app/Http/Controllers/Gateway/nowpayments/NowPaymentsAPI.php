...
class NowPaymentsAPI {
	...
	private $session, $token,$api;


	function __construct(string $token,$sandbox=true) {
		...
		if($sandbox){
		    $this->api = 'https://api-sandbox.nowpayments.io/v1/';
		}else{
		    $this->api = 'https://api.nowpayments.io/v1/';
		}
			...
			case 'GET':
				...
				if(!empty($data)) {
					if(is_array($data)) {
						...
						if($endpoint == 'payment') curl_setopt($ch, CURLOPT_URL, $this->api.$endpoint.'/'.$data);
					}
				} else {
